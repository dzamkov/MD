...
module Stream =
    ...
    let file (path : MD.Path) = new IOStream (new FileStream (path.Source, FileMode.Open), true)

    /// Constructs a stream that concatenates a series of chunks produced by the given retrieve function.
    let chunk initial retrieve = new ChunkStream<'a, 'b> (initial, retrieve)

    /// Constructs a stream that reads from the given memory region.
    let unsafe regionStart regionEnd = new UnsafeStream (regionStart, regionEnd)

/// Contains functions for constructing and manipulating data.
module Data =

    /// Constructs data for the file at the given path.
    let file (path : MD.Path) = new IOData (new FileStream (path.Source, FileMode.Open))

    /// Constructs data whose source is an IO stream.
    let io (source : System.IO.Stream) = new IOData (source)

    /// Constructs data whose source is the given memory region.
    let unsafe regionStart regionEnd = new UnsafeData (regionStart, regionEnd)
