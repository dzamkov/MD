...
type Data<'a> () = 
    ...
    /// are read.
    abstract member Lock : index : int * size : int -> Stream<'a> exclusive

    /// Creates a stream to read this data beginning at the given index.
    member this.Lock index = this.Lock (index, this.Size - index)

     /// Creates a stream to read this data.
    member this.Lock () = this.Lock (0, this.Size)
...
type BufferData<'a> (buffer : 'a[], offset : int, size : int) =
    ...
    override this.Lock (index, size) = new BufferStream<'a> (buffer, index + offset) :> 'a stream |> Exclusive.``static``

/// Data created from a series of concatenated chunks.
type ChunkData<'a> () =
    inherit Data<'a> ()
    let chunks = new System.Collections.Generic.List<int * 'a data> ()
    let mutable size = 0
    ...
    let find index =
        let rec search (index, start, stop) =
            ...
            elif index - midindex < midchunk.Size then (midchunk, midindex, index - midindex)
    ...
    override this.Read (index, buffer, offset, size) =
        ...
        let rec read (offset, size, chunk : 'a data, chunkindex, chunkoffset) totalreadsize =
            ...
            if size > chunkremsize then
                chunk.Read (chunkoffset, buffer, offset, chunkremsize)
                read (offset + chunkremsize, size - chunkremsize, snd chunks.[chunkindex], chunkindex + 1, 0) (totalreadsize + chunkremsize)
            else
    ...
    override this.Read (index, destination, size) =
        ...
        let rec read (destination, size, chunk : 'a data, chunkindex, chunkoffset) totalreadsize =
            let chunkremsize = chunk.Size - chunkoffset
            if size > chunkremsize then
                chunk.Read (chunkoffset, destination, chunkremsize)
                read (destination + nativeint (uint32 chunkremsize * itemsize), size - chunkremsize, snd chunks.[chunkindex], chunkindex + 1, 0) (totalreadsize + chunkremsize)
            else
...
type UnsafeData<'a when 'a : unmanaged> (regionStart : nativeint, regionEnd : nativeint) =
    ...
    override this.Lock (index, size) = Stream.unsafe (regionStart + nativeint (uint32 index * itemsize)) regionEnd |> Exclusive.``static``
...
module Data =
    ...
    let lock (data : 'a data) : 'a stream exclusive = data.Lock (0, data.Size)
